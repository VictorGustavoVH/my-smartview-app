...
const socket = socketIOClient('http://192.168.50.134:5000'); // Cambia a tu IP/URL real
...
const SmartViewDashboard: React.FC = () => {
  ...
  const sendCommand = async (command: string) => {
    if (!device) return;
    try {
      await fetch(`http://192.168.50.134:5000/api/devices/${device.deviceId}/command`, {
        method: 'POST',
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.header}>
        <Text style={styles.headerText}>Smart Dashboard</Text>
      </View>
      ...
      <View style={styles.footer}>
        <Text style={styles.footerText}>© 2025 Smart Dashboard. All rights reserved.</Text>
      </View>
    </View>
  );
};
